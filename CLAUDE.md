...
**Remaining before the balance pass:** **native reconnect** (built 2026-08-20, combat restore not
yet played) and the pre-duel rest *room*. The pre-duel **heal** is built and wired at
`DuelRendezvous.cs:128` — only the room-with-a-Smith version is outstanding, and `DuelArenaRest`'s
own note says the heal is what was asked for.

**The dev rig loads Workshop mods (2026-08-20).** `mods_STEAMTEST/` is read recursively and tagged
`ModSource.SteamWorkshop` *before* the Steam-initialised check, so it works with `--force-steam=off`.
It found a match-voiding incompatibility in one duel: a display mod NREs on
`CombatState.HittableEnemies` because in a duel that getter is the opposing **player**, the throw
escapes into the sim through a creature setter, and it fires a different number of times on each
peer. `DuelThirdPartyGuard` contains it. Expect this class of fault from anything that draws health
bars or damage previews — we changed what a getter the whole game reads means.
...
- **Never use `Harmony.PatchAll`.** It throws on the first bad target and silently abandons the
  ...
  cleanly` in the log on every launch, or in-game results are meaningless. **98 classes / 137 methods** (`DuelReturnToLobbyPatch` added 2026-08-18; 92/128 was read out of `logs/host.20260818T114613.log` before it) (this line said 93 and HANDOFF said 89; both were guesses, and the log is the only thing that knows) — `DuelModifierMinimumPatch` added one, and the AoE fix removed `DuelAoeProbePatch` and added `DuelAoeTargetingPatch` and `DuelHookListenerScopePatch`. Note the count is per *class*, not per patch: a class holding
