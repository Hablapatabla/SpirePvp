...
>
> **Build/test (Windows):** `.\scripts\host.ps1` (pwsh 7 — stops instances, builds, re-exports the `.pck` if assets changed, launches), then `.\scripts\client.ps1` in a second tab. macOS has `./scripts/*.sh` equivalents (no pwsh there); see `docs/MAC_SETUP.md`. A match is configured in a **Custom** lobby — the only one exposing the modifier list — so pass `-Custom` (`host.ps1`) or `--custom` (`host.sh`). **Let the host's `.pck` export finish before starting the client**; a fresh pull triggers a re-export, and reading a half-written pack fails as a `LocException` that looks exactly like malformed JSON in the repo. Console opens with `'`; `travel` unlocks clicking any map node; `unlock all` is needed per dev profile. `kill` does not work in a duel by design — use `damage <amount> <index>`.
>
