...
**Patch count: 93 classes / 129 methods** (`DuelReturnToLobbyPatch` added 2026-08-18; 92/128 was read out of a live log before it). Read out of `logs/host.20260818T114613.log`, not
...
**83 as of this handoff** (121 methods). 69/107 was confirmed against a live log on 2026-08-12.
...
| `duel draft` / `duel draft off` | **Dev shortcut, draft mode only.** Auto-takes *your* remaining picks, one per tick, in pool order. A full draft is 24 clicks before a duel can start, which is a long walk to reach the thing being tested. Safe because it picks for one player: the host still arbitrates every pick and the alternation is untouched, so it is a fast typist rather than a second path into the draft — the distinction `duel now` got wrong twice. Both players may type it. |
...
**Patch count is 83 classes / 121 methods.**
...
**Patch count: 83 classes / 121 methods.** 69/107 was verified against a live log before the last
...
work. Patch count was **69 classes / 107 methods** at the time of that note (83/121 now); confirm
